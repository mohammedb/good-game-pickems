-- Update the leaderboard function to support game type filtering
DROP FUNCTION IF EXISTS get_leaderboard(text, text);

-- Create updated leaderboard function with season and game type support
CREATE OR REPLACE FUNCTION get_leaderboard(
  time_filter text DEFAULT '', 
  season_id_param text DEFAULT NULL,
  game_type_filter text DEFAULT 'all'
)
RETURNS TABLE (
  user_id uuid,
  username text,
...
BEGIN
  ...
  END,
  CASE 
    WHEN game_type_filter IS NOT NULL AND game_type_filter != 'all'
    THEN format('AND m.game_type = %L', game_type_filter)
    ELSE ''
  END
  );
END;
...
-- Update the convenience function to support game type filtering
CREATE OR REPLACE FUNCTION get_current_season_leaderboard(
  time_filter text DEFAULT '', 
  game_type_filter text DEFAULT 'all'
)
RETURNS TABLE (
...
BEGIN
  ...
  -- Return leaderboard for the current season with game type filter
  RETURN QUERY SELECT * FROM get_leaderboard(time_filter, current_season_id, game_type_filter);
END;
$$;